...
FROM 
    Posts p
LEFT JOIN 
    UserReactions r ON p.post_id = r.post_id
WHERE 
    p.post_id = 2
GROUP BY 
...
SELECT
    ...
    ROUND(CAST(COUNT(r.reaction_id) AS NUMERIC) / COUNT(DISTINCT r.user_id), 4) AS avg_reactions_per_user
FROM
    UserReactions r
GROUP BY
...
LEFT JOIN
    UserReactions r ON p.post_id = r.post_id
WHERE
    r.reaction_date >= NOW() - INTERVAL '7 days'
GROUP BY
    p.post_id, p.post_content
ORDER BY
